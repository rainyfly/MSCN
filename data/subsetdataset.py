...
class SubSetDataSet(data.Dataset):
    def __init__(self, args, train=True):
        ...
        if args.ext == 'img':
            ...
            try:
                ...
                bin_path = os.path.join(self.dir_data, 'SUBSET_BIN_140')
            ...
            try:
                ...
                hdf5_path = os.path.join(self.dir_data, 'SUBSET_hdf')
    ...
    def _name_hrbin(self):
        return os.path.join(
            self.dir_data,
            'SUBSET_BIN_140',
            '{}_bin_subset140.npy'.format(self.split)
        )
    ...
    def _set_filesystem(self, dir_data, is_train):
        ...
        if is_train:
            self.dir_hr = os.path.join(dir_data, 'RAISE_subset_140')
            self.dir_lr = os.path.join(dir_data, 'RAISE_subset_LR_mosaic140')
        else:
            self.dir_hr = os.path.join(dir_data, 'RAISE_subset_140Vad')
            self.dir_lr = os.path.join(dir_data, 'RAISE_subset_LR_mosaic140Vad')
        self.ext = '.TIF'
